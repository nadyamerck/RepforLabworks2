    ...
    def setUpClass(cls):
        ...
        if cls.learner_defined_classes:
            for class_name, class_ in cls.learner_defined_classes.items():
                cls.learner_defined_methods[class_name] = {method_name: method for method_name, method in
                                                           inspect.getmembers(
                                                               class_, predicate=lambda x: inspect.isfunction(
                                                                   x) and not x.__name__.startswith('__'))}
